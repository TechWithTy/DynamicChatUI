    ...
    return (
        <div className="flex pl-4">
            {displayedUsers.map((user, index) => (
                <div
                    key={index}
                    className={`relative  rounded-xl w-10 h-10 border-4 border-white dark:border-gray-800 ${index !== 0 ? '-ml-2' : ''}`}
                    style={{ zIndex: displayedUsers.length - index }} // Leftmost avatar will have the highest z-index
                >
                    <div className="overflow-clip h-full w-full rounded-xl">
                        <Image
                            width={40}
                            height={40}
                            src={user.image ||
            ...
            {additionalCount > 0 && (
                <div className="relative flex items-center justify-center w-10 h-10 rounded-2xl bg-gray-300 dark:bg-gray-900 border-4 border-white dark:border-gray-800 -ml-3">
                    +{additionalCount}
                </div>
            )}
        </div>
